...
class EmailSender:
    ...
    def create_certificate_email(self, recipient_name, recipient_email, hackathon_name, certificate_path):
...
This is an automated email. Please do not reply to this message.
            ...
            if os.path.exists(certificate_path):
                with open(certificate_path, "rb") as attachment:
                    ...
                    clean_name = f"{recipient_name.replace(' ', '_')}_certificate.pdf"
                    part.add_header(
                        'Content-Disposition',
    ...
    def send_certificate(self, recipient_name, recipient_email, hackathon_name, certificate_path):
        """Send certificate to a single recipient"""
        print(f"DEBUG: Preparing to send certificate to {recipient_name} ({recipient_email})")
        
        msg = self.create_certificate_email(recipient_name, recipient_email, hackathon_name, certificate_path)
        if msg:
            return self.send_email(msg)
        return {'success': False, 'error': 'Failed to create email'}
    
    def send_bulk_certificates(self, participants_data, hackathon_name, progress_callback=None):
        """
